...
export const FamilyLockButton = ({
  ...
  <button
    type="button"
    onClick={onClick}
    title={f.cambiosPermitidos ? "Bloquear edición" : "Permitir edición"}
    className={cn(
  ...
  >
    {f.cambiosPermitidos ? <Unlock size={14} /> : <Lock size={14} />}
    <span className="text-left whitespace-pre-wrap">
      Cambios{"\n"}
      {f.cambiosPermitidos ? "permitidos" : "bloqueados"}
    </span>
  </button>
);
...
export const FamilyActionButtons = ({
  family: f,
  invitationId,
  onSendWhatsApp,
  onSendReminder,
  onDelete,
}: FamilyActionButtonsProps) => {
  ...
  const whatsappButton = () => (
    <button
      ...
      className={cn(
        ...
        (isMenuOpen || f.whatsappEnviado) &&
          "md:group-hover/wa:bg-green-100 md:group-hover/wa:text-green-700 md:group-hover/wa:border-green-600",
        isMenuOpen
  ...
  return (
    <div className="flex items-center gap-1 justify-end">
      {f.tieneTelefono && (
        <div
          className="relative flex items-center justify-center group/wa"
            ...
            <div
              className={cn(
                ...
                isMenuOpen
                  ...
                  : "opacity-0 invisible pointer-events-none scale-95 md:group-hover/wa:opacity-100 md:group-hover/wa:visible md:group-hover/wa:pointer-events-auto md:group-hover/wa:scale-100",
              )}
            >
              <div className="bg-white border border-[#EBE5DA] shadow-[0_10px_40px_-10px_rgba(44,44,41,0.2)] rounded-xl p-1.5 flex flex-col gap-1">
                ...
                >
                  ...
                  >
                    <Tooltip
                      text={`Enviado${stringFechaEnviado(f.fechaWhatsappEnviado)}`}
                      position="top"
                      align="right"
                    >
                ...
                >
                  ...
                  {f.recordatorioEnviado === true && (
                    ...
                    >
                      <Tooltip
                        text={`Enviado${stringFechaEnviado(f.fechaRecordatorioEnviado)}`}
                        position="top"
                        align="right"
                      >
      ...
      <Tooltip text="Vista previa" position="top" align="center">
        <button
          ...
          onClick={(e) =>
            stopAndRun(e, () =>
              window.open(
                `/i/${invitationId}?family=${f.id}&preview=1&token=AQWOLdldspWRKDOSAKkwqppals`,
                "_blank",
              ),
            )
          }
      ...
      <Tooltip text="Eliminar" position="top" align="center">
        <button
          type="button"
          title="Eliminar"
          onClick={(e) => stopAndRun(e, () => onDelete(f))}
