...
function TableElement({ element }: { element: SeatingElement }) {
  ...
  const isSelectedInBulk = useSeatingStore(
    (state) => state.selectedElementIds?.includes(element.id) ?? false,
  );
  ...
  const getTransformStyle = (): string | undefined => {
    ...
    if (
      ...
      return `translate3d(${deltaX / currentZoom}px, ${
        deltaY / currentZoom
      }px, 0)`;
    }
